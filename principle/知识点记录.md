...
#### 14、浏览器攻击（XSS、CSRF）[来源](https://mp.weixin.qq.com/s/sjy-Kul5y0Wj-HJwQn7gBA)
    - XSS 跨站脚本注入攻击
...
#### 23、jquery、zepto 和 vue、react的区别
  - vue数据和视图分离，解耦（开放封闭原则）
  - vue以数据驱动视图，只关心数据变化，DOM操作被封装
  - jquery等是操作真实DOM的库

  示例：[todo-list](./jqery和vue的区别)
...
#### 24、MVC 和 MVVM
  ...
  - MVVM的三要素：
    - 响应式：vue监听data变化（数据劫持 结合 发布者-订阅者模式）
    - 模板引擎：vue的模板如何被解析，指令如何处理
    - 视图渲染：vue模板如何渲染成html，及渲染过程

#### 25、数据双向绑定实现原理 [参考](https://segmentfault.com/a/1190000006599500)
