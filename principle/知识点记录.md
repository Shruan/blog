...
#### 32、[一维数组构建树结构](./算法/tree.js)
#### 33、Hybrid H5与Native通讯
  - APP向webview注入全局函数
  - H5通过iframe访问schame协议通讯
    ```js
      function nt_native (schame, cb) {
        var iframe = document.createElement('iframe')
        iframe.style.display = 'none'
        iframe.src = schame + '&callback=_native_callback'
        ...
        window['_native_callback'] = function (data) {
          cb(data)
        }
      }

      nt_native('jh://scan', function (res) {})
    ```
> 个人总结 欢迎纠正
