...
上传图片等资源压缩后再上传
  ...
  - 减少重汇重排，减少dom操作，使用className代替直接使用style编写样式、减少class选择器的层级嵌套、减少元素标签的使用
...
#### 15、原型、原型链
  ...
  ```
#### 16、数据双向绑定实现原理 [参考](https://segmentfault.com/a/1190000006599500)
...
#### 17、http协议原理
  ...
  - https 是以安全为目的的http通道，即http安全版，基于SSL加密和CA身份认证，使用非对称加密SSL进行握手（通信过程中告知后续使用的对称加密密钥），使用对称加密进行后续通信
    - 加密算法
      - 非对称加密（RSA加密） （即使用公钥加密，使用对应的私钥解密，缺点：加密解密速度效率低）
